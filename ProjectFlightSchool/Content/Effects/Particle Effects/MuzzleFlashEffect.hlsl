...
void GS_main( point VS_In input[1], inout TriangleStream<GS_Out> outputStream )
{
	...
	float3 vert[4];
	vert[0]	= input[0].position - rightVec * 0.4f - upVec * 0.4f;
	vert[1]	= input[0].position - rightVec * 0.4f + upVec * 0.4f;
	vert[2]	= input[0].position	+ rightVec * 0.4f - upVec * 0.4f;
	vert[3]	= input[0].position + rightVec * 0.4f + upVec * 0.4f;
