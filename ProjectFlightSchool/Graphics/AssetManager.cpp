...
HRESULT	AssetManager::PlaceholderAssets( ID3D11Device* device, ID3D11DeviceContext* dc )
{
	...
	if( FAILED( hr ) )
	{	
		//Failed to create the sand blend map
		return hr;
	}
	...
	hr = mResourceLoader->LoadResource( device, dc, "../Content/Assets/ForestAssets/Grass/grassasset1_DM.dds", (ID3D11Resource**)tex[7], &srv[7] );
	if( FAILED( hr ) )
	{	
		//Failed to create the water normalmap
		return hr;
	...
	Static2dAsset* waterTexture;
	waterTexture				= new Static2dAsset;
	waterTexture->mAssetId		= WATER_NORMALMAP;
	waterTexture->mFileName		= "WATER NORMALMAP";
	waterTexture->mSRV			= srv[6];

	mAssetContainer[WATER_NORMALMAP] = waterTexture;

	Static2dAsset* grassTexture;
	grassTexture				= new Static2dAsset;
	grassTexture->mAssetId		= GRASS_ASSET_TEXTURE;
	grassTexture->mFileName		= "GRASS ASSET TEXTURE";
	grassTexture->mSRV			= srv[7];

	mAssetContainer[GRASS_ASSET_TEXTURE] = grassTexture;
