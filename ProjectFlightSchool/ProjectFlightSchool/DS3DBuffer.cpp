...
HRESULT DS3DBuffer::CreateBasicBuffer( LPDIRECTSOUND8 lpDirectSound, WaveHeaderType waveFileHeader )
{
	...
	{
		hr = pDsb->QueryInterface( IID_IDirectSoundBuffer, (LPVOID*)&mBuffer );
		pDsb->Release();
	}
	return hr;
}
...
bool DS3DBuffer::FillBufferWithWave( LPDIRECTSOUND8 lpds, char *fileName )
{
	...
	hr = mBuffer->QueryInterface( IID_IDirectSound3DBuffer8, (LPVOID*)&mBuffer3D );
	if ( FAILED( hr ) )
	{
...
DS3DBuffer DS3DBuffer::DuplicateBuffer( LPDIRECTSOUND8 lpds, DS3DBuffer buffer1 )
{
	DS3DBuffer result;
...
char* DS3DBuffer::GetFileName()
{
	return mFileName;
}

int DS3DBuffer::GetID()
{
	return mID;
}
